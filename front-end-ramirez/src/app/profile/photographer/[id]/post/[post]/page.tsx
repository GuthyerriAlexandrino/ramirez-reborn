...
export default function Post() {
  ...
  async function getImageFromApi() {
    ...
    const data: PostImage = await fetch(
      `http://127.0.0.1:3001/post/${userId}/${postId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async function deletePost() {
    ...
    const data: PostImage = await fetch(
      `http://127.0.0.1:3001/posts/${postId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async function getComments() {
    ...
    const data: Comment[] = await fetch(
  ...
  async function deleteACommentaryFromPost(commentaryId: string) {
    ...
    const deletedComment: Comment = await fetch(
      `http://127.0.0.1:3001/comments/${commentaryId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
                ...
                <CommentaryCard
                  like={
                    !!comment?.likes?.filter(
                      (user) => user.user_id?.$oid === userSectionId
                    )
                  }
                  content={comment}
                  key={comment._id.$oid}
