...
export default function Search() {
  ...
  const { verifyTokenExpiration, getProfileImage } = useAuthLogin();

  useEffect(() => {
    async function getAllSpecializations() {
      // futura integração
      const data = await fetch("http://localhost:3001/specializations").then(
      ...
      <SearchPhotographerContainer>
        ...
        <SearchInputContainer>
          ...
          <AditionalInputs>
            <SelectInput
              selectName={
                search.specialization ? search.specialization : "Especialização"
              }
            >
                ...
                specializationOptions.map((item, id) => (
                  <PopupItem
                    ...
                    onClick={() =>
                      setSearch({
                        ...search,
                        specialization: item.name === "Nenhum" ? "" : item.name,
                      })
                    }
                  >
                    {item.name}
            ...
            <SelectInput
              selectName={
                search.orderBy ? selectOrderByValue : "Ordenação por:"
              }
            >
              {orderOptions.map((item, id) => (
                <PopupItem
                  ...
                  onClick={() => {
                    setSearch({
                      ...search,
                      orderBy: item.name === "Nenhum" ? "" : item.field!,
                    });
                    setSelectOrderByValue(
                      item.name === "Nenhum" ? "" : item.name
                    );
                  }}
                >
                  {item.name}
            ...
            <PriceRangeContainer>
              ...
              <GenericInput
                ...
                placeholder="Mínimo"
                onChange={(event: any) =>
                  setSearch({ ...search, minPrice: Number(event.target.value) })
                }
              />
              <Divider />
              ...
              <GenericInput
                ...
                placeholder="Máximo"
                onChange={(event: any) =>
                  setSearch({ ...search, maxPrice: Number(event.target.value) })
                }
              />
            </PriceRangeContainer>
